...
const el = {
  ...
  searchSection: document.getElementById("searchSection"), accountSection: document.getElementById("accountSection"), callSection: document.getElementById("callSection"), feedSection: document.getElementById("feedSection"),
    ...
    return `
      <article class="space-card">
        <div class="space-card-header">
          <h3 class="space-title">${escapeHtml(naturalClubTitle(space))}</h3>
          <p class="space-subtitle">${escapeHtml(space.description)}</p>
        </div>
        ...
        <div class="space-footer">
          <div class="space-metrics">
            <span class="metric-chip">◌ ${escapeHtml(String(space.waitingUsers || 0))}</span>
            <span class="metric-chip">◔ ${escapeHtml(String(space.activeUsers || 0))}</span>
            <span class="metric-chip ghost">☎ ${escapeHtml(String(space.activeCalls || 0))}</span>
          </div>
          <button class="space-join-button" data-space-id="${space.id}">${escapeHtml(t("joinSpace"))}</button>
        </div>
      </article>
    `;
  }).join("");
...
function showScreen(screen) { state.activeScreen = screen; el.searchSection.classList.toggle("hidden", screen !== "search"); el.accountSection.classList.toggle("hidden", screen !== "account"); el.callSection.classList.toggle("hidden", screen !== "call"); el.feedSection.classList.toggle("hidden", !(screen === "chat" || screen === "history")); showFeed(screen === "history" ? "history" : screen === "chat" ? "chat" : state.activeFeed); [el.navSearch, el.navAccount, el.navCall, el.navChat, el.navHistory].forEach((button) => button.classList.remove("nav-active")); if (screen === "search") el.navSearch.classList.add("nav-active"); if (screen === "account") el.navAccount.classList.add("nav-active"); if (screen === "call") el.navCall.classList.add("nav-active"); if (screen === "chat") el.navChat.classList.add("nav-active"); if (screen === "history") el.navHistory.classList.add("nav-active"); }
...
function showError(error) { console.error(error); setStatus(t("error"), t("errorHint")); }
...
el.navCall.addEventListener("click", () => showScreen("call"));
