...
function openEventStream() {
  ...
  state.eventSource.addEventListener("signal", async (event) => { const payload = JSON.parse(event.data); await ensureLocalAudio(); if (!state.peerConnection) createPeerConnection(payload.fromId); if (payload.kind === "offer") { await state.peerConnection.setRemoteDescription(new RTCSessionDescription(payload.data)); const answer = await state.peerConnection.createAnswer(); await state.peerConnection.setLocalDescription(answer); state.peerId = payload.fromId; await sendSignal(payload.fromId, "answer", answer); return; } if (payload.kind === "answer") { await state.peerConnection.setRemoteDescription(new RTCSessionDescription(payload.data)); return; } if (payload.kind === "ice-candidate" && payload.data) await state.peerConnection.addIceCandidate(new RTCIceCandidate(payload.data)); });
...
async function joinQueue() { if (!state.participantId) { setStatus(t("enterSpaceFirst"), t("defaultHint")); return; } await ensureLocalAudio(); await api("/api/queue/join", { method: "POST", body: JSON.stringify({ participantId: state.participantId }) }); setStatus(t("waitingStatus"), t("waitingHint")); await loadSpaces(state.query); }
